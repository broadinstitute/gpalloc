...
import com.google.api.services.servicemanagement.model.{
...
import org.broadinstitute.dsde.workbench.gpalloc.model.{
  AssignedProject,
  BillingProjectStatus,
  GPAllocException
}
import org.broadinstitute.dsde.workbench.gpalloc.util.Throttler
import org.broadinstitute.dsde.workbench.metrics.{
  GoogleInstrumented,
  GoogleInstrumentedService,
  Histogram
}
...
case class ConfigContents(resources: Seq[Resources])

//we're not using camelcase here because these become GCS labels, which all have to be lowercase.
case class TemplateLocation(template_org: String,
...
class HttpGoogleBillingDAO(
  ...
  val opThrottler = new Throttler(
    system,
    opsThrottle,
    opsThrottlePerDuration,
    "GoogleOpThrottler"
  )
  ...
  val credential: Credential = {
    new GoogleCredential.Builder()
      ...
      .setServiceAccountPrivateKeyFromPemFile(
        new java.io.File(serviceAccountPemFile)
      )
  ...
  private def billing: Cloudbilling = {
    new Cloudbilling.Builder(httpTransport, jsonFactory, credential)
  ...
  def servicesManager: ServiceManagement = {
    new ServiceManagement.Builder(httpTransport, jsonFactory, credential)
      .setApplicationName(appName)
      .build()
  }
  ...
  def iam: Iam = {
    new Iam.Builder(httpTransport, jsonFactory, credential)
      .setApplicationName(appName)
      .build()
  }
  ...
  def deploymentManager: DeploymentManagerV2Beta = {
    new DeploymentManagerV2Beta.Builder(httpTransport, jsonFactory, credential)
      .setApplicationName(appName)
      .build()
  }

  def gke: Container = {
    new Container.Builder(httpTransport, jsonFactory, credential)
      .setApplicationName(appName)
      .build()
  }
  ...
  private def updateGoogleBillingInfo(projectName: String,
                                      billingAccount: String) = {
    val projectResourceName = s"projects/$projectName"
    opThrottler.throttle(
      () =>
  ...
  override def transferProjectOwnership(
    project: String,
    owner: String
  ...
  override def pollOperation(
    operation: ActiveOperationRecord
  ): Future[ActiveOperationRecord] = {
    opThrottler
      .throttle(
        () =>
          retryWhen500orGoogleError(() => {
            executeGoogleRequest(
              deploymentManager
                .operations()
                .get(deploymentMgrProject, operation.operationId)
            )
          })
      )
      .map { op =>
        val errorStr = Option(op.getError).map(
          errors =>
            errors.getErrors.asScala
              .map(e => toErrorMessage(e.getMessage, e.getCode))
              .mkString("\n")
        )
  ...
  case class GoogleProjectConflict(projectName: String)
      extends GPAllocException(
        s"A google project by the name $projectName already exists",
        StatusCodes.Conflict
      )

  def getDMConfigYamlString(projectName: String,
                            dmTemplatePath: String,
                            properties: Map[String, YamlValue]): String = {
  ...
  override def cleanupDeployment(projectName: String): Future[Unit] = {
    if (cleanupDeploymentAfterCreating) {
      retryExponentially(whenDeploymentDeleteConflict) { () =>
        Future(
          blocking(
            executeGoogleRequest(
              deploymentManager
                .deployments()
                .delete(deploymentMgrProject, projectToDM(projectName))
  ...
  override def createProject(
    ...
    val templateLabels = parseTemplateLocation(dmTemplatePath)
      .map(_.toYaml)
      .getOrElse(Map("template_path" -> labelSafeString(dmTemplatePath)).toYaml)
    ...
    val yamlConfig = new ConfigFile().setContent(
      getDMConfigYamlString(projectName, dmTemplatePath, properties)
    )
    ...
    opThrottler.throttle(
          ...
          executeGoogleRequest {
            deploymentManager
              ...
              .insert(
                ...
                new Deployment()
                  .setName(projectToDM(projectName))
                  .setTarget(deploymentConfig)
              )
          }
        })
    ) map { googleOperation =>
      val errorStr = Option(googleOperation.getError).map(
        errors =>
          errors.getErrors.asScala
            .map(e => toErrorMessage(e.getMessage, e.getCode))
            .mkString("\n")
      ...
      ActiveOperationRecord(
        ...
        toScalaBool(googleOperation.getStatus == "DONE"),
        errorStr
      )
    }
  }
  ...
  private def toScalaBool(b: java.lang.Boolean) =
    Option(b).contains(java.lang.Boolean.TRUE)
  ...
  protected def cromwellAuthBucketName(bpName: String) =
    s"cromwell-auth-$bpName"

  protected def leaveThisIAMPolicy(entityName: String,
                                   projectNumber: Long): Boolean = {
  ...
  protected def cleanupPolicyBindings(projectName: String,
                                      projectNumber: Long): Future[Unit] = {
    for {
      existingPolicy <- opThrottler.throttle(
        () =>
          retryWhen500orGoogleError(() => {
            executeGoogleRequest(
              cloudResources.projects().getIamPolicy(projectName, null)
            )
          })
      )

      _ <- opThrottler.throttle(
        () =>
          retryWhen500orGoogleError(() => {
            val existingPolicies: Map[String, Seq[String]] =
              existingPolicy.getBindings.asScala.map { policy =>
                policy.getRole -> policy.getMembers.asScala
              }.toMap

            val updatedPolicies = existingPolicies
              .map {
                case (role, members) =>
                  (
                    role,
                    members.filter(
                      entityName =>
                        leaveThisIAMPolicy(entityName, projectNumber)
                    )
                  )
              }
              .collect {
                case (role, members) if members.nonEmpty =>
                  new Binding()
                    .setRole(role)
                    .setMembers(members.distinct.asJava)
              }
              .toList

            // when setting IAM policies, always reuse the existing policy so the etag is preserved.
            val policyRequest = new SetIamPolicyRequest()
              .setPolicy(existingPolicy.setBindings(updatedPolicies.asJava))
            executeGoogleRequest(
              cloudResources.projects().setIamPolicy(projectName, policyRequest)
            )
          })
      )
    } yield ()
  ...
  protected def shouldDeleteBucketACL(entityName: String): Boolean = {
    !(entityName.startsWith("project-owners-") || entityName.startsWith(
      "project-editors-"
    ))
  }

  //removes all acls added to the cromwell auth bucket that aren't the project owner/editor ones
  protected def cleanupCromwellAuthBucket(
    ...
    for {
      ...
      deleteOAcls = oAcls.getItems.asScala
        .filter(a => shouldDeleteBucketACL(a.getEntity))
      _ <- sequentially(deleteOAcls) { d =>
        googleRq(
          storage.defaultObjectAccessControls.delete(bucketName, d.getEntity)
        ).recover {
          // ignore any errors where the group is already gone
          case ge: GoogleJsonResponseException
              if ge.getStatusCode == 400 && ge.getMessage.contains(
      ...
      deleteBAcls = bAcls.getItems.asScala
        .filter(a => shouldDeleteBucketACL(a.getEntity))
      _ <- sequentially(deleteBAcls) { d =>
        googleRq(storage.bucketAccessControls.delete(bucketName, d.getEntity))
          .recover {
            // ignore any errors where the group is already gone
            case ge: GoogleJsonResponseException
                if ge.getStatusCode == 400 && ge.getMessage.contains(
  ...
  protected def retryForPetDeletion(
    op: () => Unit
  )(implicit histo: Histogram): Future[Unit] = {
  ...
  def overPetLimit(projectName: String): Future[Boolean] = {
    for {
      serviceAccounts <- googleRq(
        iam.projects().serviceAccounts().list(gProjectPath(projectName))
      )
      pets = googNull(serviceAccounts.getAccounts)
        .filter(_.getEmail.contains(s"@$projectName.iam.gserviceaccount.com"))
    } yield {
      pets.size > maxPets
    }
  }
      ...
      _ <- sequentially(clusterNames) { clusterName =>
        googleRq(
          dataproc
            ...
            .regions()
            .clusters()
            .delete(projectName, "us-central1", clusterName)
        ).recover {
          // ignore errors on already deleting clusters
          case ge: GoogleJsonResponseException
              if ge.getStatusCode == 400 && ge.getMessage.contains(
                "while it has other pending delete operations"
              ) =>
        }
      }
    } yield {}
  ...
  def cleanupVMs(projectName: String): Future[Unit] =
    for {
      result <- googleRq(
        computeManager.instances().list(projectName, "us-central1-a")
      )
      ...
      _ <- sequentially(instanceNames) { instanceName =>
        googleRq(
          computeManager
            .instances()
            .delete(projectName, "us-central1-a", instanceName)
        )
      }
    } yield {}
  ...
  def cleanupDisks(projectName: String): Future[Unit] =
    for {
      result <- googleRq(
        computeManager.disks().list(projectName, "us-central1-a")
      )
      disks = googNull(result.getItems)
      diskNames = disks.map(i => i.getName)
      _ <- sequentially(diskNames) { diskName =>
        googleRq(
          computeManager.disks().delete(projectName, "us-central1-a", diskName)
        )
      }
    } yield {}
  ...
  protected def createStorageLogsBucket(
    billingProjectName: String
  ): Future[String] = {
    ...
    retryWithRecoverWhen500orGoogleError(() => {
      ...
      bucket.setLifecycle(
        new Lifecycle().setRule(List(storageLogExpiration).asJava)
      )
  ...
  protected def allowGoogleCloudStorageWrite(bucketName: String): Unit = {
    ...
    val bac = new BucketAccessControl()
      .setEntity("group-cloud-storage-analytics@google.com")
      .setRole("WRITER")
